     ...
     <div className="tools-card tools-input"><div className="tools-card-title"><FolderOpen size={18}/><div><strong>{tr('Input sources', 'Источники')}</strong><small>{tr('One demo or many folders', 'Одно демо или несколько папок')}</small></div></div>
       ...
       <fieldset className="tools-dataset" disabled={locked}>
         ...
         <div className="tools-dataset-options">
           <label><input type="radio" name="parser-source" value="raw" checked={source === 'raw'} onChange={() => setSource('raw')} /><span>{tr('All throws', 'Все броски')}</span></label>
           <label><input type="radio" name="parser-source" value="canonical" checked={source === 'canonical'} onChange={() => setSource('canonical')} /><span>{tr('Deduplicated', 'Без повторов')}</span></label>
         </div>
       </fieldset>
        <div className="tools-actions"><button className="btn" disabled={locked} onClick={() => void action(async()=>{await invoke('deduplicate_parser_workspace'); await refresh();})}><RefreshCw size={14}/>{tr('Recompute', 'Пересчитать')}</button><button className="btn" disabled={locked} onClick={() => void action(async()=>{const p=await invoke<string|null>('save_parser_output',{source,format:'json'});if(p)setMessage(`${tr('Saved', 'Сохранено')}: ${p}`);})}><FileJson size={14}/>JSON</button><button className="btn" disabled={locked} onClick={() => void action(async()=>{const p=await invoke<string|null>('save_parser_output',{source,format:'msgpack'});if(p)setMessage(`${tr('Saved', 'Сохранено')}: ${p}`);})}>MPK</button></div>
        <button className="btn primary tools-import" disabled={locked || !counts[0]} onClick={() => void action(async () => { const p = await invoke<string>('prepare_parser_import', { source }); await importJson(p); await refreshImports?.(); setMessage(tr('Imported into Viewer', 'Импортировано в Viewer')); })}>{tr('Import selected dataset into Viewer', 'Импортировать выбранный набор')}</button>
