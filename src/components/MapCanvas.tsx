...
const splitCommands = (value?: string | null) => value
  ? value.replace(/\s+(setang|setpos)\b/gi, "\n$1").split(/[;\n]+/).map((part) => part.trim()).filter(Boolean).slice(0, 3)
  : [];

type ThrowGroup = { id: string; x: number; y: number; grenades: GrenadePreview[] };
const groupThrowPoints = (items: GrenadePreview[], mode: "throw" | "landing"): ThrowGroup[] => {
  const buckets = new Map<string, { x: number; y: number; sx: number; sy: number; grenades: GrenadePreview[] }>();
  items.forEach((grenade) => {
    ...
    if (current && Math.hypot(current.x - x, current.y - y) <= 8) {
      ...
      buckets.set(current ? `${key}:${grenade.id}` : key, { x, y, sx: x, sy: y, grenades: [grenade] });
    }
  });
  return [...buckets.values()].map((group) => {
    const grenades = [...group.grenades].sort((a, b) => Number(b.is_core) - Number(a.is_core) || b.usage_count - a.usage_count || a.id - b.id);
    return { id: grenades.map((grenade) => grenade.id).join("-"), x: group.x, y: group.y, grenades };
  });
};
...
export default function MapCanvas({
  ...
  const [preview, setPreview] = useState<{ grenade: GrenadePreview; x: number; y: number } | null>(null);
  ...
  const groups = useMemo(() => groupThrowPoints(grenades, grenadePointMode), [grenades, grenadePointMode]);
  ...
  return <div className={`map-canvas icon-theme-${iconTheme}`} data-tour="map-canvas">
    ...
    <details className="map-legend" data-map-control="1">
      ...
      <div className="map-legend-content">
        ...
        <span><i className="legend-dot smoke" /> {tr("Smoke", "Смок")}</span>
        <span><i className="legend-dot flash" /> {tr("Flash", "Флешка")}</span>
        <span><i className="legend-dot molotov" /> {tr("Molotov", "Молотов")}</span>
    ...
    <div ref={viewportRef} className={`map-viewport ${view.s > 1 ? "is-draggable" : ""}`} onMouseDownCapture={onMouseDown} onClickCapture={suppressDraggedClick} onDragStart={(event) => event.preventDefault()} onWheel={onWheel}>
      <div ref={stageRef} className="map-stage" style={stageSize.width && stageSize.height ? { width: stageSize.width, height: stageSize.height } : undefined}>
        <div className="map-world" style={{ transform: `translate(${view.tx}px, ${view.ty}px) scale(${view.s})` }}>
          ...
          {selectedCluster ? <div className="map-caption"><span>{count(selectedCluster.count, "grenade selected", "grenades selected", "граната выбрана", "гранаты выбрано", "гранат выбрано")}</span><span>{selectedCluster.unique_types.map(grenadeLabel).join(", ")}</span></div> : null}
        </div>
      ...
      {activeGroup ? <div className="throw-strip" data-map-control="1" onWheel={(event) => { event.preventDefault(); event.currentTarget.scrollLeft += event.deltaY; }}>{activeGroup.grenades.map((grenade) => <button key={grenade.id} className={`throw-strip-dot ${grenade.is_core ? "core" : ""}`} style={{ "--dot": typeColor[grenade.grenade_type] ?? "#fff" } as CSSProperties} onClick={() => onGrenadeOpen?.(grenade.id)} onPointerEnter={(event) => showPreview(event, grenade)} onPointerLeave={() => setPreview(null)}>{markerIcon(grenade.grenade_type)}</button>)}</div> : null}
       {preview ? <div className="throw-preview-popover" style={previewStyle} data-map-control="1"><div className="throw-preview-head"><span className="throw-preview-mark" /><strong>#{preview.grenade.id} {grenadeLabel(preview.grenade.grenade_type)}</strong><span>{preview.grenade.side}</span>{isInstaGrenade(preview.grenade, spawnMapPoints) ? <em className="insta">{INSTA_LABEL}</em> : null}{preview.grenade.is_core ? <em>{tr("Core", "Core")}</em> : null}</div>{previewKeys.length ? <div className="throw-preview-keys">{previewKeys.map((key, index) => { const visual = throwKeyVisual(key); return <span className="throw-preview-key-part" key={`${key}-${index}`}><span className={`throw-preview-key-icon ${visual.kind}`} data-tip={visual.title}><ThrowKeyIcon glyph={visual.glyph} /><span>{visual.label}</span></span>{index < previewKeys.length - 1 ? <span className="throw-preview-plus">+</span> : null}</span>; })}</div> : <div className="throw-preview-empty">{tr("No throw keys", "Нет клавиш броска")}</div>}<div className="throw-preview-info"><span className="throw-preview-thrower">{preview.grenade.thrower || tr("Parsed throw", "Распознанный бросок")}</span><span>{tr("Airtime", "Время полета")}: <strong>{typeof preview.grenade.airtime === "number" ? `${preview.grenade.airtime.toFixed(2)}s` : "-"}</strong></span><span>{tr("Usage", "Использований")}: <strong>{formatNumber(preview.grenade.usage_count)}</strong></span></div>{previewCommands.length ? <div className="throw-preview-command">{previewCommands.map((command) => <span key={command}>{command}</span>)}</div> : null}</div> : null}
    </div>
  </div>;
}
